...
#include <cstdio>
#include <cstring>
#include <libcore/devices/internet_socket.hpp>
...
class Esp8266 : public Module<>
{
 public:
  ...
  void ModuleInitialize() override
  {
    ...
    try
    {
      log::Debug("Reseting module...");
      WifiWrite("+++");
      Reset();

      // Disable echo
      log::Debug("Disabling echo back...");
      WifiWrite("ATE0\r\n");
      ReadUntil(kOk, kDefaultTimeout, true);

      // Enable simple IP client mode
      log::Debug("Set device to simple IP client mode...");
      WifiWrite("AT+CWMODE=1\r\n");
      ReadUntil(kOk, kDefaultTimeout, true);

      log::Debug("Testing that module is ready...");
      TestModule();
    }
